...
export default function BackupManagerModal({
  ...
  const handleExportPlainVault = async () => {
    if (confirm('⚠️ ATENÇÃO (LGPD & Segurança): O arquivo em texto plano salvará suas senhas sem criptografia. Deseja continuar?')) {
      setIsExporting(true);
      setError('');
      try {
    ...
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0, 0, 0, 0.85)', backdropFilter: 'blur(8px)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '1rem', zIndex: 100 }}>
      <div className="glass-panel animate-fade-in" style={{ maxWidth: '540px', width: '100%', padding: '2rem', position: 'relative' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.6rem', marginBottom: '1.5rem' }}>
          <div style={{ padding: '0.6rem', background: 'rgba(0, 245, 160, 0.12)', border: '1px solid rgba(0, 245, 160, 0.3)', borderRadius: 'var(--radius-sm)' }}>
            <Download size={22} style={{ color: 'var(--accent-emerald)' }} />
          </div>
          <div>
            <h2 style={{ fontSize: '1.25rem', fontWeight: 700 }}>Gerenciador de Duplo Backup</h2>
            <span style={{ fontSize: '0.78rem', color: 'var(--text-muted)' }}>Backups isolados por conta e conformidade com LGPD</span>
          </div>
        </div>

        {/* Notifications */}
        {error && (
          <div style={{ padding: '0.75rem 1rem', background: 'rgba(255, 42, 109, 0.15)', border: '1px solid var(--color-danger)', borderRadius: 'var(--radius-sm)', color: 'var(--text-primary)', fontSize: '0.85rem', marginBottom: '1rem' }}>
            {error}
          </div>
        )}

        {successMessage && (
          <div style={{ padding: '0.75rem 1rem', background: 'rgba(0, 245, 160, 0.15)', border: '1px solid var(--color-success)', borderRadius: 'var(--radius-sm)', color: 'var(--accent-emerald)', fontSize: '0.85rem', display: 'flex', alignItems: 'center', gap: '0.5rem', marginBottom: '1rem' }}>
            <Check size={18} />
            {successMessage}
          </div>
        )}
        ...
        <div style={{ marginBottom: '1.5rem', padding: '1rem', background: 'rgba(0, 242, 254, 0.05)', border: '1px solid rgba(0, 242, 254, 0.2)', borderRadius: 'var(--radius-md)' }}>
        ...
        <div style={{ marginBottom: '1.5rem', padding: '1rem', background: 'rgba(255, 255, 255, 0.03)', border: '1px solid var(--border-light)', borderRadius: 'var(--radius-md)' }}>
